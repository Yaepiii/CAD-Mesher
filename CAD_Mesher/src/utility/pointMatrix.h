...
public:
    ...
    void addPoint(const Eigen::Matrix<double, 3, 1> & point_add){
        ...
        variance(0, num_point) = 100;   //TODO：根据range对不确定性进行赋值
        num_point++;
    }

    void addPointWithCurvature(const Eigen::Matrix<double, 3, 1> & point_add, const std::vector<double> curvature){
        //add a point
        if( num_point+1 > point.cols()){
            point.conservativeResize(Eigen::NoChange_t(3), point.cols()+resize_step);
            variance.conservativeResize(Eigen::NoChange_t(1), point.cols() + resize_step);
        ...
        point.col(num_point) = point_add;
        if(int(curvature[num_point]) / 100 < 1.0){
            variance(0, num_point) = int(curvature[num_point]);
        }
        else if(100 * (curvature[num_point] - int(curvature[num_point])) < 0.8){
            variance(0, num_point) = (int(curvature[num_point]) / 100 + (curvature[num_point] - int(curvature[num_point])) * 100) / 2;
        }
        else if(((curvature[num_point]) / 100 < 1.0) && ((curvature[num_point] - int(curvature[num_point])) * 100 < 0.8)){
            variance(0, num_point) = (curvature[num_point] - int(curvature[num_point])) * 100;
        }
        else{
    ...
    PointMatrix copyAndTransform(Transf & transf){
        ...
        tmp_point_expend = transf * tmp_point_expend;   //变换到世界坐标系
    ...
    void transformPoints(Transf & trans){
        ...
        tmp_point_expend.bottomRows(1).fill(1);
        tmp_point_expend = trans * tmp_point_expend;    //变换到世界坐标系
        point.leftCols(num_point) = tmp_point_expend.topRows(3);
    }

    //PCA分解，求解最小特征值和最小特征向量(平面法线方向)
    void eigenDecomposition(){
        ...
        if(num_point != 0){
            ...
            eigenvalMat = eig.pseudoEigenvalueMatrix(); //特征值矩阵
            eigenvectorMat = eig.pseudoEigenvectors();  //特征向量从小到大排序
