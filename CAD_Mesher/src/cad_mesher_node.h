#ifndef CAD_Mesher_NODE_H
#define CAD_Mesher_NODE_H

#endif //CAD_Mesher_NODE_H
#include "cell.h"
#include "map.h"
...
public:
    ...
    double keyframe_adding_distance, keyframe_adding_angle, stable_threshold, p_hit, p_miss;
    int scan_number, skip_map_glb_pub, ground_number_threshold;
    bool use_adaptive_kd, use_curvature, use_curvature_weight, use_dynamic_removal;
    float image_res;
    float V_FOV_UP;
    float V_FOV_DOWN;
    float V_FOV;
...
public:
    ...
    Eigen::Matrix<double, 1, Eigen::Dynamic> num_cells_now, num_cells_glb, num_cells_new ,
            ...
            time_down_sample, time_devide_point, time_dynamic_removal, time_sliding_window, 
    ...
    bool receive_first_imu = false, imu_init = false, if_first_trans_init = false;
    int step = 0;
    //transformPoints
    ...
    //point cloud
    pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_submap_ptr;
    pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_static_ptr;
    pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_dynamic_ptr;
    pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_new_ptr;
    pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_pre_ptr;
    std::deque<std::pair<Transf, pcl::PointCloud<pcl::PointXYZ>::Ptr>> sliding_window;
    double spaciousness;
...
public:
    ...
    mesh_pub , mesh_pub_local, mesh_voxblox_pub,
    aft_mapping_odom_highfrec_pub, aft_mapping_highfrec_odom_path_pub;
    image_transport::Publisher range_image_now_pub, range_image_submap_pub;

    ros::Subscriber pointcloud_sub, laser_sub, odom_sub, imu_sub, imu_raw_sub, alt_sub;

    nav_msgs::Path aft_mapping_highfrec_odom_path;

    std::mutex mBuf;
    std::thread MappingThread, SpaciousnessThread;
    sensor_msgs::ImagePtr rangeImage_now_msg, rangeImage_submap_msg;
    ...
    void map2odomUpdate();
    void pubTf();
    CAD_Mesher(ros::NodeHandle & nh_, Parameter & param_, Log & g_data_);
    ~CAD_Mesher();
    void process();
    ...
    void setDSAdaptiveParams();
    //project pointcloud to range image
    cv::Mat projectPointCloud(const pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_in, cv::Mat & range_image_idx);
    cv::Mat projectPointCloud2(const pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_in, cv::Mat & range_image_idx);
    std::vector<int> getStaticIdxFromDynamicIdx(const std::vector<int>& dynamic_point_idx, int all_points_number);
    void parseStaticMapPointcloudUsingPtIdx(std::vector<int>& point_idx);
    void parseDynamicMapPointcloudUsingPtIdx(std::vector<int>& point_idx);
    static void abort(){
        abort_ = true;
    }
};