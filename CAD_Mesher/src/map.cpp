...
extern Parameter param;//in CADMesher node
extern Log g_data;//in CAD-Mesher node

pcl::PointCloud<pcl::PointXYZ>::Ptr getPointloudWithCurvature(pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_raw, std::vector<double>& curvature_output){
    int cloudSize = pcl_raw->points.size();
    double startOri = -atan2(pcl_raw->points[0].y, pcl_raw->points[0].x);
    double endOri = -atan2(pcl_raw->points[cloudSize - 1].y, pcl_raw->points[cloudSize - 1].x) + 2 * M_PI;
    ...
    int count = cloudSize;
    pcl::PointXYZ point;
    std::vector<pcl::PointCloud<pcl::PointXYZ>> pcl_scans(param.scan_number);
    for(int i = 0; i < cloudSize; ++i){
        point.x = pcl_raw->points[i].x;
        point.y = pcl_raw->points[i].y;
        point.z = pcl_raw->points[i].z;

        double verticalAngle = atan(point.z / sqrt(point.x * point.x + point.y * point.y)) * 180 / M_PI;
        int scanID = 0;

        if(param.scan_number == 16){
            scanID = int ((verticalAngle + 15) / 2 + 0.5);
            if(scanID > (param.scan_number - 1) || scanID < 0){
                count--;
                continue;
            }
        }
        else if(param.scan_number == 32){
        ...
        else if(param.scan_number == 64){
            if(verticalAngle >= -8.83){
                scanID = int((2 - verticalAngle) * 3.0 + 0.5);
            }
            else{
                scanID = param.scan_number / 2 + int((-8.83 - verticalAngle) * 2.0 + 0.5);
            }
            if(verticalAngle > 2 || verticalAngle < -24.33 || scanID > param.scan_number - 1 || scanID < 0){
                count--;
                continue;
            }
        }
        ...
        pcl_scans[scanID].push_back(point);
    }
    cloudSize = count;
    pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_cur(new pcl::PointCloud<pcl::PointXYZ>());
    for(int i = 0; i < param.scan_number; ++i){
        *pcl_cur += pcl_scans[i];
    }

    std::vector<double> range_tmp(cloudSize);
    #pragma omp parallel for num_threads(param.num_thread) default(shared)
    for(int i = 0; i < cloudSize; ++i){
        range_tmp[i] = sqrt(pcl_cur->points[i].x * pcl_cur->points[i].x + pcl_cur->points[i].y * pcl_cur->points[i].y + pcl_cur->points[i].z * pcl_cur->points[i].z);
    }
    // median
    std::nth_element(range_tmp.begin(), range_tmp.begin() + range_tmp.size()/2, range_tmp.end());
    double median_curr = range_tmp[range_tmp.size() / 2];  // 中值
    ...
    // std::vector<double> curvature_result(cloudSize, -1);
    pcl::PointCloud<pcl::PointXYZ>::Ptr pcl_output(new pcl::PointCloud<pcl::PointXYZ>());
    int corner_number = 0;
    #pragma omp parallel for num_threads(param.num_thread) default(shared)
    for(int i = 5; i < cloudSize - 5; ++i){
        double diffX = pcl_cur->points[i - 5].x + pcl_cur->points[i - 4].x + pcl_cur->points[i - 3].x + pcl_cur->points[i - 2].x + pcl_cur->points[i - 1].x - 10 * pcl_cur->points[i].x + pcl_cur->points[i + 1].x + pcl_cur->points[i + 2].x + pcl_cur->points[i + 3].x + pcl_cur->points[i + 4].x + pcl_cur->points[i + 5].x;
        double diffY = pcl_cur->points[i - 5].y + pcl_cur->points[i - 4].y + pcl_cur->points[i - 3].y + pcl_cur->points[i - 2].y + pcl_cur->points[i - 1].y - 10 * pcl_cur->points[i].y + pcl_cur->points[i + 1].y + pcl_cur->points[i + 2].y + pcl_cur->points[i + 3].y + pcl_cur->points[i + 4].y + pcl_cur->points[i + 5].y;
        double diffZ = pcl_cur->points[i - 5].z + pcl_cur->points[i - 4].z + pcl_cur->points[i - 3].z + pcl_cur->points[i - 2].z + pcl_cur->points[i - 1].z - 10 * pcl_cur->points[i].z + pcl_cur->points[i + 1].z + pcl_cur->points[i + 2].z + pcl_cur->points[i + 3].z + pcl_cur->points[i + 4].z + pcl_cur->points[i + 5].z;
        double dis_factor = 2.0 / (1.0 + range_tmp[i] / 20.0);
        if (dis_factor < 0.2) dis_factor = 0.2;
        double curvature = sqrt(diffX * diffX + diffY * diffY + diffZ * diffZ) * dis_factor / (10 * sqrt(pcl_cur->points[i].x * pcl_cur->points[i].x + pcl_cur->points[i].y * pcl_cur->points[i].y + pcl_cur->points[i].z * pcl_cur->points[i].z));
        curvature = ((double)((int)((curvature + 0.005) * 100))) / 100;

        double diff_range = range_tmp[i - 5] + range_tmp[i - 4] + range_tmp[i - 3] + range_tmp[i - 2] + range_tmp[i - 1] - 10.0 * range_tmp[i] + range_tmp[i + 1] + range_tmp[i + 2] + range_tmp[i + 3] + range_tmp[i + 4] + range_tmp[i + 5];
        double curvature2 = abs(diff_range) * dis_factor / (10 * range_tmp[i]);
        curvature2 = ((double)((int)((curvature2 + 0.005) * 100))) / 100;
        
        if ((curvature > 1.8 || curvature2 > 2.0) && corner_number < cloudSize * 0.03){
            corner_number++;
            continue;
        }
        #pragma omp critical
        pcl_output->points.push_back(pcl_cur->points[i]);
        #pragma omp critical
        curvature_output.push_back(curvature / 100 + curvature2 * 100);
    }
    return pcl_output;
}
...
bool getPointCloud(PointMatrix & points_result, pcl::PointCloud<pcl::PointXYZ> & pcl_got, double & get_laser_time,
    ...
    if(param.use_curvature){
        pcl_filtered_ptr = getPointloudWithCurvature(pcl_filtered_ptr, curvature);
    }
    ...
    if(param.use_curvature && param.use_curvature_weight){
        if(range_max <= 100){
            ...
            for(int i = 0; i < points_raw.num_point; i += filter_point_step){
                ...
                if(range_sqaure < range_max_square && range_sqaure > range_min_sqaure ){
                    Eigen::Matrix<double, 4, 1> point_add_with_curvature;
                    point_add_with_curvature.topRows(3) = points_raw.point.col(i);
                    point_add_with_curvature(3, 0) = 100;
                    if(int(curvature[i]) / 100 < 1.0 && int(curvature[i]) / 100 > 0.1){
                        point_add_with_curvature(3, 0) = int(curvature[i]);
                    }
                    else if(100 * (curvature[i] - int(curvature[i])) < 0.8 && 100 * (curvature[i] - int(curvature[i])) > 0.1){
                        point_add_with_curvature(3, 0) = (int(curvature[i]) / 100 + (curvature[i] - int(curvature[i])) * 100) / 2;
                    }
                    else if(((curvature[i]) / 100 < 1.0) && ((curvature[i]) / 100 > 0.1) && ((curvature[i] - int(curvature[i])) * 100 < 0.8) && ((curvature[i] - int(curvature[i])) * 100 > 0.1)){
                        point_add_with_curvature(3, 0) = (curvature[i] - int(curvature[i])) * 100;
                    }
                    // points_result.addPointWithCurvature(points_raw.point.col(i), curvature); //too computationally-dense, unusage!
                    points_result.addPointWithVariance(point_add_with_curvature);
                }
            }
        }
        else{
...
void  Map::dividePointsIntoCellInitMap(PointMatrix & points_raw){
    ...
    Cell init_cell;
    // init_cell.p_stable -= odds(param.p_miss);
    ...
    for(int i_bucket = 0; i_bucket < index_bucket_enough_point.size(); i_bucket ++){
        ...
        Cell tmp_cell(points_raw_cell, g_data.step,
                      posi_tmp, region_tmp,
                      true, true);
        ary_bucket_cell[index_bucket_enough_point[i_bucket]] = tmp_cell;
    }
...
bool  Map::processNewScan(Transf & Tguess, int step_, const Map & map_glb){
    ...
    points_turned.transformPoints(Tguess);  //变换到世界坐标系
    dividePointsIntoCell(points_turned, map_glb, true); //GP
    return true;
}
OverlapCellsRelation Map::overlapCellsFull(Map & map_glb){
    // find overlapped cells between map_glb and map_now
    ROS_DEBUG("overlapCellsFull");
    // TicToc t_overlap_region;
    OverlapCellsRelation overlap_ship;
    //g_data.un_bounded_count = 0;

    for(int i = 0; i < cells_now.size(); i++){
        std::pair<double, Cell> & i_cell_now = cells_now[i];
            ...
            for(int i = -1 * overlap_lace; i <= overlap_lace; i++) {
                ...
                if(index == 0){
                    for (int i_point = 0; i_point < point_now.num_point; i_point++) {
                        ...
                        } else if (dis_square < closest_point_glb(4, i_point)) {
                            closest_point_glb.col(i_point).topRows(3) = points_glb.point.col(i_point);
                            closest_point_glb.col(i_point).row(3) = points_glb.variance.col(i_point);
                            closest_point_glb(4, i_point) = dis_square;
                        }
                ...
                for (int i_point = 0; i_point < point_now.num_point; i_point++) {
                    if ((points_glb.variance(0, i_point) < variance_thr)) { //variance_thr=0.6
                        double dis_square = (points_glb.point(dir, i_point) - point_now.point(dir, i_point)) *
                                            (points_glb.point(dir, i_point) - point_now.point(dir, i_point));
                        if (closest_point_glb(4, i_point) < 0) {
                            closest_point_glb.col(i_point).topRows(3) = points_glb.point.col(i_point);
...
Transf Map::computeTPointToMesh(Map & map_glb, Map & map_now){
    ...
    for(int dir = 0; dir < 3; dir ++){
        PointMatrix & scan_glb = map_glb.ary_overlap_vertices[dir]; //findMatchPointToMesh()赋值
        PointMatrix & scan_now = map_now.ary_overlap_vertices[dir];
    ...
    for(int i = 0; i < overlap_ship.cells_now.size(); i++){
        if((overlap_ship.cells_glb[i] != nullptr) &&
           (overlap_ship.cells_now[i] != nullptr)){
            ...
            if(param.use_dynamic_removal && cell_now.odds_stable < param.stable_threshold){
                // ROS_ERROR("------------------------------");
                // std::cout << "cell_now.odds_stable: " << cell_now.odds_stable << std::endl;
                continue;
            }
...
void Map::dynamicRemoval(Map map_glb){
    //find overlapped regions
    OverlapCellsRelation overlap_ship = this->overlapCellsFull(map_glb);
    // for(auto [key, value] : this->cells_now){
    //     value.odds_stable -= odds(param.p_miss);
    // }
    //update overlapped region
    //#pragma omp parallel for num_threads(param.num_thread)
    for(int i = 0; i < overlap_ship.cells_now.size(); i++){
        if((overlap_ship.cells_glb[i]->empety == false) &&
           (overlap_ship.cells_now[i]->empety == false)){
            Cell & cell_glb = *overlap_ship.cells_glb[i];
            Cell & cell_now = *overlap_ship.cells_now[i];
            cell_glb.odds_stable += odds(param.p_hit);
            cell_now.odds_stable += odds(param.p_hit);
           }
        else if((overlap_ship.cells_glb[i]->empety == true) &&
           (overlap_ship.cells_now[i]->empety == false)){
            Cell & cell_glb = *overlap_ship.cells_glb[i];
            Cell & cell_now = *overlap_ship.cells_now[i];
            cell_glb.odds_stable += odds(param.p_miss);
            cell_now.odds_stable += odds(param.p_miss);
           }
        else if((overlap_ship.cells_glb[i]->empety == false) &&
           (overlap_ship.cells_now[i]->empety == true)){
            Cell & cell_glb = *overlap_ship.cells_glb[i];
            Cell & cell_now = *overlap_ship.cells_now[i];
            cell_glb.odds_stable -= odds(param.p_miss);
            cell_now.odds_stable -= odds(param.p_miss);
...
void Map::fineDynamicRemoval() {
    // fine dynamic removal
    ROS_DEBUG("fineDynamicRemoval");

    for(int i = 0; i < this->cells_now.size(); i++){
        std::pair<double, Cell> & i_cell_now = this->cells_now[i];
        double posi_now = i_cell_now.second.hash_position;
        double posi_now_x, posi_now_y, posi_now_z;
        decodePosiWithTime(posi_now, posi_now_x, posi_now_y, posi_now_z, param.grid);
        for(int j = 0; j < index_bucket_enough_point.size(); j++){
            // auto i_cell_glb = this->cells_now.find(index_bucket_enough_point[j]);
            std::pair<double, Cell> & i_cell_occupy = this->cells_now[index_bucket_enough_point[j]];
            double posi_occupy = i_cell_occupy.second.hash_position;
            double posi_occupy_x, posi_occupy_y, posi_occupy_z;
            decodePosiWithTime(posi_occupy, posi_occupy_x, posi_occupy_y, posi_occupy_z, param.grid);
            if( (std::abs(posi_now_x - posi_occupy_x) <= 3 * param.grid && std::abs(posi_now_y - posi_occupy_y) <= 3 * param.grid && 
                std::abs(posi_now_z - posi_occupy_z) <= 3 * param.grid) &&
                posi_now_x * posi_now_x + posi_now_y * posi_now_y + posi_now_z * posi_now_z
                < posi_occupy_x * posi_occupy_x + posi_occupy_y * posi_occupy_y + posi_occupy_z * posi_occupy_z
                ){
                i_cell_now.second.odds_stable -= odds(param.p_miss);
            }
            else{
                i_cell_now.second.odds_stable += odds(param.p_hit);
            }
            i_cell_occupy.second.odds_stable += odds(param.p_hit);
        }
    }
}
