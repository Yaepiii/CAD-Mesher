...
extern Parameter param;//in CAD-Mesher node
...
void Cell::gaussianProcess(enum Direction gp_direction){
    ...
    for(int row = 0; row < num_train; row++){
        ky(row, row) = k(row, row) + variance_sensor * variance_sensor; // \sigma^2 * I + Kll
    }
    ...
    kky.block(0, 0, num_test_square, num_train) = k_starm.block(0, 0, num_test_square, num_train) *
                                                  (ky.block(0, 0, num_train, num_train).llt().solve(I_train));
    ...
    if(bound_point){
        for(int i_point = 0; i_point < num_test_square; i_point++){
            //prediction should not exceed the voxel region
...
void Cell::reconstructSurfaces(bool glb_cell_not_surface){
    ...
    if(cell_raw_points.eig_sorted.rbegin()->first / (++cell_raw_points.eig_sorted.begin())->first > eig_3ratio2_not_surface){//48
        ...
        //can form surface(s)
        not_surface = false;
        if((++sorted_angle_a.begin()) ->first - sorted_angle_a.begin()->first > eig_2minus1_distinctive_angle){//0.95
...
void Cell::updateVertices(Cell & cell_new, enum Direction update_direction){
    ...
    int num_test_square = param.num_test * param.num_test;
    double variance_map_update = param.variance_map_update; //0.5
